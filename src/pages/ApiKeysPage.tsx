...
export const ApiKeysPage: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      const [keysData, orgData] = await Promise.all([
        apiService.getApiKeys(),
        apiService.getOrganization(),
      ]);
      setApiKeys(keysData);
      setEnvironments(orgData.environments);
    } catch (err) {
  ...
  const handleCreateKey = async () => {
    ...
    try {
      const result = await apiService.createApiKey(selectedEnvId);
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-200">
            {apiKeys.length === 0 ? (
              ...
              apiKeys.map((key) => (
                <tr key={key.id}>
                  <td className="px-4 py-3 text-sm text-gray-900">{key.environment}</td>
            ...
            {createdKey ? (
              <>
                ...
                <div className="mb-4">
                  ...
                  <div className="p-3 bg-gray-50 border border-gray-200 rounded font-mono text-sm break-all">
                    {createdKey}
                  </div>
                </div>
                <button
